...
class SeriesRecommendationServiceTest extends AbstractUnitTest {
    ...
    @Mock
    private SeriesCatalogService seriesCatalogService;

    @InjectMocks
    private SeriesRecommendationService seriesRecommendationService;

    @Test
    void should_returnEmptyBuckets_when_userHasNoSeriesPreferences() {
        var user = TestFixtures.user(1L, "testuser");
        var catalog = List.of(TestFixtures.catalogItem("Breaking Bad"));

        when(preferenceRepository.findByUser(user)).thenReturn(List.of());
        when(seriesCatalogService.findRecommendationCatalog()).thenReturn(catalog);

        RecommendationResponse response = seriesRecommendationService.recommendedForYou(user, 4);
    ...
    @Test
    void should_returnUnavailable_when_userHasFewRatedSeries() {
        var user = TestFixtures.user(1L, "testuser");

        when(userMediaRepository.findByUser(eq(user), any(Pageable.class)))
                .thenReturn(new PageImpl<>(List.of()));
        when(seriesCatalogService.findRecommendationCatalog())
                .thenReturn(List.of(TestFixtures.catalogItem("Breaking Bad")));

        MaybeYouLikeResult result = seriesRecommendationService.maybeYouLike(user, 4, Set.of());
