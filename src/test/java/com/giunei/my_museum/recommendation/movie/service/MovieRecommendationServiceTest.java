...
class MovieRecommendationServiceTest extends AbstractUnitTest {
    ...
    @Test
    void should_returnEmptyBuckets_when_userHasNoMoviePreferences() {
        var user = TestFixtures.user(1L, "testuser");
        var catalog = List.of(TestFixtures.catalogItem("Matrix"));

        when(preferenceRepository.findByUser(user)).thenReturn(List.of());
        when(movieCatalogService.findRecommendationCatalog()).thenReturn(catalog);

        RecommendationResponse response = movieRecommendationService.recommendedForYou(user, 4);
    ...
    @Test
    void should_returnUnavailable_when_userHasFewRatedMovies() {
        var user = TestFixtures.user(1L, "testuser");

        when(userMediaRepository.findByUser(eq(user), any(Pageable.class)))
                .thenReturn(new PageImpl<>(List.of()));
        when(movieCatalogService.findRecommendationCatalog())
                .thenReturn(List.of(TestFixtures.catalogItem("Matrix")));

        MaybeYouLikeResult result = movieRecommendationService.maybeYouLike(user, 4, Set.of());
